import { ProjectStatus, type Deliverable, type MeetingRecord, type Settings } from "@prisma/client";
...
export function computeHealth(
  deliverables: Pick<Deliverable, "completed" | "targetDate">[],
  ...
  // Condition A — deliverable slippage.
  const overdue = deliverables.filter((d) => !d.completed && d.targetDate.getTime() < now.getTime());
  const weeksLate = (d: { targetDate: Date }) => (now.getTime() - d.targetDate.getTime()) / WEEK_MS;
  const badlyLate = overdue.filter((d) => weeksLate(d) >= settings.weeksBehindMilestone);
  const conditionA = badlyLate.length > 0;
  if (conditionA) {
    reasons.push(
      `${badlyLate.length} deliverable${badlyLate.length === 1 ? "" : "s"} more than ${settings.weeksBehindMilestone} week(s) overdue`,
    );
  }
  ...
  if (overdue.length > 0) {
    return {
      ...
      reasons: [`${overdue.length} deliverable(s) overdue`],
    };
  }
  if (streak === 1) {
...
export async function recomputeProjectStatus(projectId: string): Promise<HealthResult & { changed: boolean }> {
  const [project, settings, deliverables, recentMeetings] = await Promise.all([
  ...
  const result = computeHealth(deliverables, recentMeetings, settings);
